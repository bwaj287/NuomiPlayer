...
- 根据当前真机测试，酷狗桌面歌词在“未锁定”状态下更容易被抓到；锁定后经常会切成不可读的纯显示浮层
- 当前桌面歌词抓取仍有一个已知问题：部分歌曲，尤其是英文歌，可能出现“隔一行漏一行”的现象
...
- 酷狗桌面歌词如果被“锁定”在桌面，当前版本往往抓不到；解除锁定后通常可以恢复抓取。
- 酷狗桌面歌词浮层对无障碍暴露的文本并不总是完整，当前版本仍可能漏掉部分歌词行。
...
1. 在酷狗音乐里开始播放
2. 尽量停留在“歌词实际可见”的酷狗前台页面，或者开启酷狗桌面歌词
3. 如果使用酷狗桌面歌词，建议保持“未锁定”状态
4. 打开手机上的小老王播放器，确认底部状态已经显示捕获到播放器
5. 如有需要，开启 `车载常亮`
   说明：10 秒无操作后会自动把小老王播放器当前窗口调暗，触摸后恢复亮度
6. 连接 Android Auto，在媒体应用列表中打开小老王播放器
...
- 现阶段如果你的目标是“稳定抓歌词”，优先推荐酷狗前台歌词页或“未锁定”的桌面歌词；锁定桌面歌词更适合纯显示，不适合当前抓取方案
- 如果后续继续迭代歌词稳定性，下一阶段更可能转向“时序跟踪”或 `OCR` 路线，而不是继续堆叠单帧无障碍打分
